...
public class RCTBluetoothSerialModule extends ReactContextBaseJavaModule
  ...
  private static final int REQUEST_COARSE_LOCATION_PERMISSION = 11;
  // Members
  ...
  public void handleSearch(Promise promise) {
    ...
    if (mBluetoothAdapter != null) {
      boolean shouldAskLocation = shouldAskLocationPermission();
  ...
  @ReactMethod
  public void writeTextToDevice(String Text, String encoding, Promise promise) {
    byte[] send;
    try {
      send = Text.getBytes(encoding);
    } catch (Exception e) {
      send = Text.getBytes();
    }
  ...
  private boolean shouldAskLocationPermission() {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
      if (!isLocationPermitted() && getCurrentActivity() != null) {
